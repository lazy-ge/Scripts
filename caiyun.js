/*
该脚本基于Peng-YM的caiyun.js改写,不需要进入box.js内填入彩云天气API、腾讯地图API、经纬度,使用苹果自带天气APP获取当前定位
此脚本必须下载到本地文件夹中,并在该脚本第28行的双引号中填入彩云天气API,第29行双引号中填入腾讯地图API
配置：
1. 配置自动定位
根据平台添加如下配置
(1). Quantumult X
...
async function query() {
  const now = new Date();
    // query API
    const url = `https://api.caiyunapp.com/v2.5/${config.caiyun_api}/${$.read("location").longitude},${$.read("location").latitude}/weather?lang=zh_CN&dailystart=0&hourlysteps=384&dailysteps=16&alert=true`;
    ...
    const address =
      await $
        .get(`https://apis.map.qq.com/ws/geocoder/v1/?key=${config.tencent_api}&location=${$.read("location").latitude},${$.read("location").longitude}`)
        .then(resp => {
          const body = JSON.parse(resp.body);
          if (body.status !== 0) {
